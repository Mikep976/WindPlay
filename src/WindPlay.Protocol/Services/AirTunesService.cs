    ...
    ReceiverIdentity identity) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            if (!_rtspConnections.TryAdd(remoteEndPoint, connection))
            {
                connection.Dispose();
                continue;
            }
    ...
    {
        _tcpListener.Stop();
        foreach (RtspConnection connection in _rtspConnections.Values)
            connection.Dispose();
        _rtspConnections.Clear();
...
internal static partial class AirTunesServiceLoggers
{
    ...
    [LoggerMessage(LogLevel.Warning, "Rejected RTSP client [{endPoint}] due to the local-network or connection-limit policy")]
    public static partial void RtspClientRejected(this ILogger logger, EndPoint? endPoint);
}
