...
public class RtspHeadersCollection : IEnumerable<RtspHeader>
{
    ...
    {
        if (_headers.TryGetValue(key, out RtspHeader? existing))
        {
    ...
    public bool TryGetSingleValue(string key, [NotNullWhen(true)] out string? value)
    {
        if (_headers.TryGetValue(key, out RtspHeader? header) && header.Values.Length == 1)
        {
